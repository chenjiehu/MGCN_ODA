...
from Dataset.Dataset import SOURCE_DATA, TARGET_DATA, QUERY_DATA, get_shot_data
from samplers import CategoriesSampler
from utils import pprint, ensure_path, Averager, count_acc, euclidean_metric, cal_acc2, find_p_less_index, Averager_vector, find_p_greater_index, NUM_shot
from wBDA import coral_wBDA
from model.Multi_gcn import MultiGCN_relation
from EntropyLoss import EntropyLoss
from model.backbone import Conv4
from model.convert import Convnet
from coral import compute_aifa_W
from utils import Auxiliary_data, Task_data
from utils import save_path_step1, save_path_step2, save_path_DA, ensure_path,set_gpu, filename_shot, filename_query, Common_class_number
...
if __name__ == '__main__':
    ...
    parser.add_argument('--repeat-number', type=int, default=NUM_shot)
    ...
    parser.add_argument('--save-path', default=save_path_DA)
    parser.add_argument('--load', default=save_path_step2)
    parser.add_argument('--gpu', default='1')
    parser.add_argument('--same-class-num', default=Common_class_number)
    ...
    model2_select_target = MultiGCN_relation(input_dim=1600, N_way=args.test_way).cuda()
    ...
    for ii in range(0,args.repeat_number):
        ...
        optimizer_CNN = torch.optim.Adam(model_CNN.parameters(), lr=0.0001)
        lr_scheduler_CNN = torch.optim.lr_scheduler.StepLR(optimizer_CNN, step_size=20, gamma=0.5)
        optimizer_GCN = torch.optim.Adam(model_GCN.parameters(), lr=0.0001)
        ...
        valset = QUERY_DATA(os.path.join(filename_query,'5_query'+str(ii)+'.csv'))
