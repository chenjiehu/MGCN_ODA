...
from Dataset import SOURCE_DATA, TARGET_DATA, get_shot_data,QUERY_DATA
from samplers import CategoriesSampler
from utils import pprint, set_gpu, Averager, Timer, count_acc, euclidean_metric, cal_acc2, Averager_vector
from utils import step2_save_path
from Multi_gcn import MGCN
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load', default=step2_save_path)   #close denotes the closed set DA
    ...
    with torch.no_grad():
        ...
        for ii in range(0,NUM_shot):
            ...
            target_shot_data, target_shot_label = get_shot_data('./shot_OA/5_shot' + str(ii) + '.csv')
            target_shot_data = target_shot_data.cuda()
            target_shot_label = target_shot_label.cuda()

            valset = QUERY_DATA('./query_OA/5_query' + str(
                ii) + '.csv')
