...
import torch
import os
from utils import shot, filename, filename_shot, filename_query, root, ensure_path
from Dataset import SOURCE_DATA
#
def Split_shot_query(root,filename,file_shot,file_query,shot, num_split = 10):

    filename = filename
    for loop in range(0,num_split):
        filename_shot = file_shot + str(loop) + '.csv'
        filename_query = file_query + str(loop) + '.csv'
...
if __name__=='__main__':
    ...
    Split_shot_query(root, filename,filename_shot,filename_query,shot)
    pass
